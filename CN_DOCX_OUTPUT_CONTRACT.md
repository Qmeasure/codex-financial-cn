...
本文件是 `financial-services-cn` 的中文 Word 交付契约。任何技能生成 DOCX、Word 或面向 Google Docs 导入的 `.docx` 文件时，都必须在用户模板和品牌规范之外遵守本契约。用户模板优先，但不得省略中文可读性、来源脚注、币种、单位、日期、口径、免责声明和人工复核边界。
...
captions:
  ...
  source: {size: 7.5pt, color: "#666666"}
```
...
- 免责声明必须包含“不构成投资、法律、税务、会计或监管建议”和“需专业人员复核”含义。
...
- 如果来源来自订阅终端且没有公开 URL，必须写明终端或数据库名称、数据日期和“需终端复核”。
