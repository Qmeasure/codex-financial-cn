...
- 缺少任务 2？→ 先完成任务 2（财务建模）
- 缺少任务 3？→ 先完成任务 3（估值分析）
- 缺少外部数据？→ 先填写数据源发现记录；Gate 通过且覆盖不足时再从 Yahoo Finance、Bloomberg 或类似来源收集，并说明降级原因
...
```python
# chart_01：股价表现（12-24 个月）
# - 折线图，展示该股票 12-24 个月原始股价走势，不做归一化，不重设起点为 100
# - 图内标注最新价格、52 周区间、数据日期和来源
# - 用于最终报告第 1 页标题和必要公司元数据下方、摘要表或正文表格之前
```
