...
凡任务需要外部数据、最新财报、行情、一致预期、业绩会、公告或监管文件，任何网页搜索、公司官网抓取、SEC/交易所抓取前，必须先生成“数据源发现记录”。

未生成该记录，不得进入网页搜索、公司官网抓取、SEC/交易所抓取，也不得写正式报告。

数据源发现记录必须包含：
1. 当前会话可用的 MCP / connector / 已授权数据库 / 本地数据源；
2. 本任务适用与不适用的数据源判断；
3. 已尝试调用的数据源及结果，或无法调用的具体原因；
4. 仍缺失的数据项；
5. 是否允许降级到官方披露 + 网页辅助，以及降级理由。

最终报告必须在“数据来源与口径说明”中摘要列示该记录。若用户明确说“不用 MCP，只用官方公告”，也必须在记录中说明用户覆盖了默认数据源顺序，并只使用官方披露。
...
- [ ] DOCX 文件存在
- [ ] DOCX 可打开或结构校验通过
- [ ] 已生成数据源发现记录，且记录列明 MCP/connector/授权源检查结果
- [ ] “数据来源与口径说明”包含数据源发现记录摘要
- [ ] DOCX 包含 8-12 张图表
- [ ] DOCX 包含 1-3 张摘要表
- [ ] DOCX 首页标题和必要公司元数据下方包含 12-24 个月原始股价图，且图内标注最新价格、52 周区间、数据日期和来源
...
- 假设读者已经看过首次覆盖报告
- 正式 DOCX 首页必须在标题和必要公司元数据下方放置该股票 12-24 个月原始股价图；不做归一化，并在图内标注最新价格、52 周区间、数据日期和来源。

### 4. 引用与来源归属 ⭐⭐⭐ 强制

**关键**：所有数据都必须用具体来源和可点击超链接正确引用。

**每个 Exhibit 都要包含带可点击链接的单一主来源：**
...
- [ ] 正式表格中的缺失市场数据或一致预期单元格显示 `—`，不写固定校验标签
...
### 阶段 4：报告创建（2-3 小时）
按章节顺序创建目标篇幅 8-12 页 DOCX 报告。页数是最终篇幅目标，不是插入显式分页的指令；章节应连续排版，由 Word 自然分页。

完整章节结构、目标篇幅和格式要求见 [references/report-structure.md](references/report-structure.md)。

**高层结构：**
- 业绩摘要章节：含评级和目标价，建议篇幅约 1 页
- 详细业绩分析章节：建议篇幅约 2 页
- 关键指标与指引章节：建议篇幅约 2 页
- 更新后的投资逻辑章节：建议篇幅约 2 页
- 估值与预测章节：建议篇幅约 2-3 页
- 附录章节：可选，建议篇幅约 1-2 页
