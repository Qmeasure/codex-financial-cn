---
name: deal-screening
description: 根据基金投资标准快速筛选流入项目，包括 CIM、teaser 和中介材料。提取关键交易指标，运行通过/失败框架，并输出一页式筛选备忘录。适用于审阅新项目、分流流入材料或判断是否参加首次电话会议。触发词包括“筛选这个项目”、“审阅这个 CIM”、“这个值得看吗”、“分流这个 teaser”或“项目筛选”。
---
...
- **交易类型**：Platform、add-on、recap、少数股权、carve-out
- **要价/估值**：倍数，如披露则包括企业价值
- **卖方动机**：为什么现在出售
- **管理层**：继续 rollover 还是退出
- **关键客户**：集中度风险
- **关键风险**：明显红旗
...
1. **结论**：通过 / 进一步尽调 / 明确放弃
2. **Bull case**（2-3 条）：为什么这可能是一笔好交易
3. **Bear case**（2-3 条）：关键风险和顾虑
4. **关键问题**：首次电话会议需要回答什么

### 第 4 步：输出

输出一页式筛选备忘录，适合与合伙人分享或用于 IC 快速筛选。
