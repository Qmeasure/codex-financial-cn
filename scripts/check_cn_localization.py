...
def contains_forbidden_legacy_text(text: str, forbidden: str) -> bool:
    if re.fullmatch(r"[A-Za-z0-9_-]+", forbidden):
        return re.search(rf"\b{re.escape(forbidden)}\b", text, flags=re.IGNORECASE) is not None
...
def check_manifest() -> None:
    ...
    if claude.get("name") != PLUGIN_NAME:
        err("Claude Code plugin.json name 必须是 financial-services-cn")
    if not has_cjk(claude.get("description", "")):
        err("Claude Code plugin.json description 必须是中文")
    for key in ("version", "homepage", "repository", "license"):
        if claude.get(key) != data.get(key):
    ...
    if marketplace.get("name") != PLUGIN_NAME:
        err("Claude Code marketplace name 必须是 financial-services-cn")
    if not has_cjk(marketplace.get("description", "")):
    ...
    if entry.get("name") != PLUGIN_NAME:
        err("Claude Code marketplace 插件 name 必须是 financial-services-cn")
    if entry.get("source") != "./":
        err("Claude Code marketplace 插件 source 必须是 ./")
    if not has_cjk(entry.get("description", "")):
...
def check_forbidden_and_residual_text() -> None:
    for path in iter_text_files():
        ...
        for forbidden in FORBIDDEN_LEGACY_WORDING:
            if contains_forbidden_legacy_text(text, forbidden):
